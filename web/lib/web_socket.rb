...
class WebSocketClient
    ...
    def initialize(arg, params = {})
      if params[:server] # server
        ...
        line = gets()
        if !line
          raise(WebSocketClient::Error, "Client disconnected without sending anything.")
        end
        line = line.chomp()
    ...
    def handshake(status = nil, header = {})
      if @handshaked
        raise(WebSocketClient::Error, "handshake has already been done")
      end
...
class WebSocketServer
    ...
    def create_web_socket(socket)
      ...
      if ch == ?<
        ...
        return WebSocketClient.new(socket, :server => self)
      end
    end
...
if __FILE__ == $0
  ...
  if ARGV[0] == "server" && ARGV.size == 3
    ...
    client = WebSocketClient.new(ARGV[1])
    puts("Connected")
    Thread.new() do
